...
def create_data_loaders(args,motion=False):
    dev_data, train_data = create_datasets(args)
    display_data = [dev_data[i] for i in range(0, len(dev_data), len(dev_data) // 8)]

    train_loader = DataLoader(
        dataset=train_data,
        batch_size=5 if motion else args.batch_size,
        shuffle=True,
...
def evaluate(args, epoch, models, data_loader, writer, dl_len, ratio, train_loss=None, train_rec_loss=None, train_vel_loss=None,
    ...
    with torch.no_grad():
        if epoch != 0:
            for iter, data in data_loader:
                ...
                motion_output = input.sum(dim=-1)
                for i in range(motion_output.shape[1]):
                    motion_output[:, i, :, :] = models[0](input[:, i, :, :].unsqueeze(1)).squeeze(1)

                static_output = models[1](input)

                # output = transforms.complex_abs(output)  # complex to real
                # output = transforms.root_sum_of_squares(output, dim=1)
                # output = output.squeeze()
                output = ratio*motion_output+(1-ratio)*static_output
            ...
            x = ratio*models[0].get_trajectory() + (1-ratio)*models[1].get_trajectory()
            v, a = get_vel_acc(x)
...
def visualize(args, epoch, models, data_loader, writer,ratio):
    ...
    with torch.no_grad():
        for iter, data in enumerate(data_loader):
            ...
            if epoch != 0:
                ...
                static_output = models[1](input.clone())
                motion_output = motion_output.unsqueeze(2)
                static_output = static_output.unsqueeze(2)
                # output = transforms.complex_abs(output)  # complex to real
                # output = transforms.root_sum_of_squares(output, dim=1).unsqueeze(1)

                motion_corrupted = models[0].subsampling(input).unsqueeze(2)
                static_corrupted = models[1].subsampling(input).unsqueeze(2)
                corrupted = (1-ratio)*static_corrupted + ratio*motion_corrupted
...
def train():
    ...
    if args.resume:
        ...
        static_model = build_model(args)
        if args.data_parallel:
            motion_model = torch.nn.DataParallel(motion_model)
            static_model = torch.nn.DataParallel(static_model)
        motion_optimizer = build_optim(args, motion_model)
        static_optimizer = build_optim(args, static_model)
        best_dev_loss = 1e9
        start_epoch = 0
    logging.info(args)
    # logging.info(model)
    models = [motion_model,static_model]
    train_loader, dev_loader, display_loader = create_data_loaders(args,motion = False)
    motion_train_loader,_,_ = create_data_loaders(args, motion=True)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        start = time.time()
        for model in models:
            tr_loader = enum_motion if model.motion else enum_train
            tr_len = len(motion_train_loader) if model.motion else len(train_loader)
            optimizer = motion_optimizer if model.motion else static_optimizer
            train_loss, train_time, train_rec_loss, train_vel_loss, train_acc_loss, psnr_train = train_epoch(args,
                                                                                                             epoch,
                                                                                                             model,
                                                                                                             tr_loader,
                                                                                                             optimizer,
        ...
        dev_loss, dev_time, psnr_dev = evaluate(args, epoch + 1, models, enum_val, writer, len(dev_loader),ratio,
                                                train_loss,
                                                train_rec_loss, train_vel_loss, train_acc_loss, psnr_train)

        visualize(args, epoch + 1, models, display_loader, writer,ratio)
        ...
        if dev_loss < best_dev_loss:
            ...
            is_new_best = False
        save_model(args, args.exp_dir, epoch, motion_model, motion_optimizer, best_dev_loss, is_new_best)
        logging.info(
