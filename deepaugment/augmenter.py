...
def transform(aug_type, magnitude, X,seed=42):
    if aug_type == "crop":
        ...
        X_aug = iaa.GaussianBlur(sigma=(0, magnitude * 25.0),seed=seed).augment_images(X)
    elif aug_type == "rotate":
        rot = np.random.randint(-180,180)
        X_aug = iaa.Affine(rotate=(rot, rot),seed=seed).augment_images(X)
    elif aug_type == "shear":
        shear = np.random.randint(-45,45)
        X_aug = iaa.Affine(shear=(shear, shear),seed=seed).augment_images(X)
    elif aug_type == "translate-x":
        X_aug = iaa.Affine(
            translate_percent={"x": (-magnitude, magnitude), "y": (0, 0)},seed=seed
        ).augment_images(X)
    elif aug_type == "translate-y":
        X_aug = iaa.Affine(
            translate_percent={"x": (0, 0), "y": (-magnitude, magnitude)},seed=seed
        ).augment_images(X)
    elif aug_type == "horizontal-flip":
        X_aug = iaa.Fliplr(magnitude,seed=seed).augment_images(X)
    elif aug_type == "vertical-flip":
        X_aug = iaa.Flipud(magnitude,seed=seed).augment_images(X)
    elif aug_type == "sharpen":
        X_aug = iaa.Sharpen(
            alpha=(0, 0.05), lightness=(0.50, 5 * magnitude),seed=seed
        ).augment_images(X)
    elif aug_type == "emboss":
        X_aug = iaa.Emboss(
            alpha=(0, 1.0), strength=(0.0, 20.0 * magnitude),seed=seed
        ).augment_images(X)
    elif aug_type == "additive-gaussian-noise":
        X_aug = iaa.AdditiveGaussianNoise(
            loc=0, scale=(0.0, magnitude * 255), per_channel=0.5,seed=seed
        ).augment_images(X)
    elif aug_type == "dropout":
        X_aug = iaa.Dropout(
            (0.01, max(0.011, magnitude)), per_channel=0.5,seed=seed
        ).augment_images(
            X
        ...
        X_aug = iaa.CoarseDropout(
            (0.03, 0.15), size_percent=(0.30, np.log10(magnitude * 3)), per_channel=0.2,seed=seed
        ).augment_images(X)
    elif aug_type == "gamma-contrast":
        X_norm = normalize(X)
        X_aug_norm = iaa.GammaContrast(magnitude * 1.75,seed=seed).augment_images(
            X_norm
        )  # needs 0-1 values
        X_aug = denormalize(X_aug_norm)
    elif aug_type == "brighten":
        X_aug = iaa.Add(
            (int(-40 * magnitude), int(40 * magnitude)), per_channel=0.5,seed=seed
        ).augment_images(
            X
        )  # brighten
    elif aug_type == "invert":
        X_aug = iaa.Invert(1.0,seed=seed).augment_images(X)  # magnitude not used
    elif aug_type == "fog":
        X_aug = iaa.Fog(seed=seed).augment_images(X)  # magnitude not used
    elif aug_type == "clouds":
        X_aug = iaa.Clouds(seed=seed).augment_images(X)  # magnitude not used
    elif aug_type == "histogram-equalize":
        X_aug = iaa.AllChannelsHistogramEqualization(seed=seed).augment_images(
            X
        )  # magnitude not used
        ...
        X_norm = normalize(X)
        X_aug_norm = iaa.PerspectiveTransform(seed=seed,
        ...
        X_aug_norm2 = iaa.ElasticTransformation(
            alpha=(0.0, max(0.5, magnitude * 300)), sigma=5.0,seed=seed
        ).augment_images(X_norm2)
        X_aug_norm = (X_aug_norm2 + 1) / 2
        X_aug = denormalize(X_aug_norm)
        ...
        X_aug = iaa.Flipud(magnitude).augment_images(X_aug)
    elif aug_type == "coarse-salt-pepper":
        X_aug = iaa.CoarseSaltAndPepper(p=0.2, size_percent=magnitude,seed=seed).augment_images(X)
...
def augment_by_policy(
    ...
    for i in range(0,len(hyperparams)-1,4):
        ...
        X_portion_aug = 255*(X_portion_aug-X_portion_aug.min())/(X_portion_aug.max()-X_portion_aug.min())
        assert (
